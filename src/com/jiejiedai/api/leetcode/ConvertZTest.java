        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (inArray(c, num)) {
                if (c == ' ') {
                    ...
                    isStart = true;
                    int cn = isM ? Character.getNumericValue(c) : -Character.getNumericValue(c);
                    if ((result < Integer.MAX_VALUE / 10 || (result == Integer.MAX_VALUE / 10 && cn <= Integer.MAX_VALUE % 10)) &&
                            (result > Integer.MIN_VALUE / 10 || (result == Integer.MIN_VALUE / 10 && cn >= Integer.MIN_VALUE % 10))) {
                        result = result * 10 + cn;
                    } else {
                        result = isM ? Integer.MAX_VALUE : Integer.MIN_VALUE;
                        break;
                    }
                }
...
//    }
    public static boolean isPalindrome(int x) {
        ...
        while (x != 0) {
            ...
            if (y < Integer.MAX_VALUE / 10 || (y == Integer.MAX_VALUE / 10 && l < Integer.MAX_VALUE % 10)) {
                y = y * 10 + l;
            } else {
                return (x1 / 10 == y) && (l == x1 % 10);
            }
        }
...
//    }
    ...
    public String intToRoman(int num) {
        ...
        num = toRo(num, 1000, "M", "", "", sb);
        num = toRo(num, 100, "C", "D", "M", sb);
        num = toRo(num, 10, "X", "L", "C", sb);
        num = toRo(num, 1, "I", "V", "X", sb);
//        if (num / 1000 != 0) {
...
//        }
    ...
    private static int toRo(int num, int c, String s1, String s5, String s10, StringBuilder sb) {
        int b = num / c;
        if (b == 4) {
            sb.append(s1).append(s5);
        } else if (b == 9) {
            sb.append(s1).append(s10);
        } else {
            if (b / 5 != 0) {
                sb.append(s5);
                b = b % 5;
    ...
    public int romanToInt(String s) {
        ...
        for (int i = 0; i < romans.length; i++) {
            String ro = romans[i];
            while (s.startsWith(ro)) {
                s = s.substring(ro.length());
                result += ints[i];
            }
        }
        return result;
...
//
//        Arrays.sort(nums);
//       retry1: for (int i = 0; i < nums.length; i++) {
//            int n1 = nums[i];
...
//                break;
//            }
//            retry2:  for (int j = i + 1; j < nums.length - 1; j++) {
//                int n2 = nums[j];
...
//    }
            ...
            if (i == 0 || nums[i - 1] != nums[i]) {
                ...
                for (int j = i + 1; j < nums.length - 1; j++) {
                    if (j == i + 1 || nums[j - 1] != nums[j]) {
                        int b = nums[j];
                        while (j < cI && a + b + nums[cI] > 0) {
                            cI--;
                        }
                        if (cI > j && a + b + nums[cI] == 0) {
                            List<Integer> list = new ArrayList<>();
                            list.add(a);
                            list.add(b);
                            list.add(nums[cI]);
        ...
        for (int i = 0; i < nums.length - 2; i++) {
            if (i == 0 || nums[i - 1] != nums[i]) {
                ...
                for (int j = i + 1; j < nums.length - 1; j++) {
                    if (j == i + 1 || nums[j - 1] != nums[j]) {
                        int b = nums[j];
                        while (j < cI && a + b + nums[cI] > target) {
                            cI--;
                        }

                        if (cI + 1 > j) {
                            int c;
                            if (cI == j || (cI + 1 < nums.length && Math.abs(a + b + nums[cI] - target) > Math.abs(a + b + nums[cI + 1] - target))) {
                                c = nums[cI + 1];
                            } else {
                                c = nums[cI];
                            }
                            ...
                            if (Math.abs(a + b + c - target) < Math.abs(result - target)) {
    ...
    public List<String> letterCombinations(String digits) {
        ...
        int l = 0;
        for (int i = 0; i < chars.length; i++) {
            List<String> strings = map.get(chars[i]);
            l = strings.size();
    ...
    public static void main(String[] args) {
        System.out.println(new ConvertZTest().letterCombinations("23"));


//        int reverse = reverse(-120);
//        System.out.println(reverse);
//        System.out.println(myAtoi("4"));
//        System.out.println(myAtoi("   49"));
//        System.out.println(myAtoi("4193 with words4"));
//        System.out.println(myAtoi("words and 987"));
//        System.out.println(myAtoi("-+91283472332"));
//        System.out.println(myAtoi("214-74836472asdfasfd"));
